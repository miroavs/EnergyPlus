...
class NFA {
 ...
 private:
  ...
  void AddToThreadq(Threadq* q, int id0, int c, re2::StringPiece  context,
  ...
  int Step(Threadq* runq, Threadq* nextq, int c, re2::StringPiece  context,
           const char* p);
...
// context is used (with p) for evaluating empty-width specials.
// p is the current input position, and t0 is the current thread.
void NFA::AddToThreadq(Threadq* q, int id0, int c, re2::StringPiece  context,
                       const char* p, Thread* t0) {
  if (id0 == 0)
    return;
...
// Frees all the threads on runq.
// If there is a shortcut to the end, returns that shortcut.
int NFA::Step(Threadq* runq, Threadq* nextq, int c, re2::StringPiece  context,
              const char* p) {
  nextq->clear();
...
bool NFA::Search(re2::StringPiece  text, re2::StringPiece  const_context,
            bool anchored, bool longest,
            StringPiece* submatch, int nsubmatch) {
  if (start_ == 0)
    return false;
...
bool
Prog::SearchNFA(re2::StringPiece  text, re2::StringPiece  context,
