...
Cell::Cell(std::size_t index, const CellType cellType, std::size_t i,
           std::size_t j, std::size_t k, std::size_t *stepsize,
...
void Cell::setDistances(double dxp_in, double dxm_in, double dyp_in,
                        double dym_in, double dzp_in, double dzm_in) {
  dist[0][1] = dxp_in;
  dist[0][0] = dxm_in;
  dist[1][1] = dyp_in;
  dist[1][0] = dym_in;
...
void Cell::calcCellMatrix(Foundation::NumericalScheme scheme, double timestep,
...
void Cell::calcCellADI(std::size_t dim, double timestep, const Foundation &foundation,
...
void Cell::gatherCCoeffs(double theta, bool cylindrical, double (&C)[3][2]) {
  for (auto dim : dims) {
    if (dim < 5) {
...
ExteriorAirCell::ExteriorAirCell(std::size_t index, const CellType cellType,
...
void ExteriorAirCell::calcCellADI(std::size_t /*dim*/, double ,
                                  const Foundation &, const BoundaryConditions &bcs, double &A,
                                  double (&)[2], double &bVal) {
  doOutdoorTemp(bcs, A, bVal);
}

void ExteriorAirCell::calcCellMatrix(Foundation::NumericalScheme, double ,
                                     const Foundation &, const BoundaryConditions &bcs, double &A,
...
InteriorAirCell::InteriorAirCell(std::size_t index, const CellType cellType,
                                 std::size_t i, std::size_t j, std::size_t k,
                                 std::size_t *stepsize, const Foundation &foundation,
                                 Surface *surfacePtr, Block *blockPtr, Mesher *meshPtr)
    : Cell(index, cellType, i, j, k, stepsize, foundation, surfacePtr, blockPtr, meshPtr) {}
...
void InteriorAirCell::calcCellMatrix(Foundation::NumericalScheme, double ,
                                     const Foundation &, const BoundaryConditions &bcs, double &A,
                                     double (&)[3][2], double &bVal) {
  doIndoorTemp(bcs, A, bVal);
}

void InteriorAirCell::calcCellADI(std::size_t /*dim*/, double ,
                                  const Foundation &, const BoundaryConditions &bcs, double &A,
                                  double (&)[2], double &bVal) {
  doIndoorTemp(bcs, A, bVal);
}
...
BoundaryCell::BoundaryCell(std::size_t index, const CellType cellType, std::size_t i,
                           std::size_t j, std::size_t k, std::size_t *stepsize,
...
void BoundaryCell::calcCellADI(std::size_t dim, double ,
                               const Foundation & /*foundation*/, const BoundaryConditions &bcs,
...
void BoundaryCell::calcCellMatrix(Foundation::NumericalScheme, double ,
                                  const Foundation & /*foundation*/, const BoundaryConditions &bcs,
                                  double &A, double (&Alt)[3][2], double &bVal) {
  std::size_t dim = surfacePtr->orientation_dim;
  std::size_t dir = surfacePtr->orientation_dir;
...
void BoundaryCell::zfCellADI(int dim, int sdim, int sign, double &A,
...
void BoundaryCell::ifCellADI(int dim, int sdim, int dir, double &A, double &Alt, double &bVal) {
  INTFLUX_PREFACE
...
void BoundaryCell::efCellADI(int dim, int sdim, int dir, double &A, double &Alt, double &bVal) {
  EXTFLUX_PREFACE
...
void BoundaryCell::ifCellMatrix(int dim, int dir, double &A, double &Alt, double &bVal) {
  INTFLUX_PREFACE
...
void BoundaryCell::efCellMatrix(int dim, int dir, double &A, double &Alt, double &bVal) {
  EXTFLUX_PREFACE
...
void BoundaryCell::zfCellADEUp(std::size_t dim, std::size_t dir, double &U) {
  if (dir == 1) {
...
void BoundaryCell::ifCellADEUp(int dim, int dir, double &U) {
  INTFLUX_PREFACE
...
void BoundaryCell::efCellADEUp(int dim, int dir, double &U) {
  EXTFLUX_PREFACE
...
void BoundaryCell::zfCellADEDown(std::size_t dim, std::size_t dir, double &V) {
  if (dir == 1) {
    V = *(&V + stepsize[dim]);
  } else /* if (dir == 0) */ {
...
void BoundaryCell::ifCellADEDown(int dim, int dir, double &V) {
  INTFLUX_PREFACE
...
void BoundaryCell::efCellADEDown(int dim, int dir, double &V) {
  EXTFLUX_PREFACE
...
double BoundaryCell::zfCellExplicit(std::size_t dim, std::size_t dir) {
  int sign = (dir == 0) ? -1 : 1;
  return *(told_ptr + sign * stepsize[dim]);
}

double BoundaryCell::ifCellExplicit(std::size_t dim, std::size_t dir) {
  INTFLUX_PREFACE
...
double BoundaryCell::efCellExplicit(std::size_t dim, std::size_t dir) {
  EXTFLUX_PREFACE
...
ZeroThicknessCell::ZeroThicknessCell(std::size_t index, const CellType cellType,
                                     std::size_t i, std::size_t j,
                                     std::size_t k, std::size_t *stepsize,
